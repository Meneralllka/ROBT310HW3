...
3 (5%) - for implementation of a different shape (circular, parallelogram, etc…) -- DONE
...
6 (10%) For questions 1‐5, create a video describing your solutions. -- DONE
'''
...
def closest_image_par (img_list, base_cut, mask):
    ideal = None
    overall_comp = None
    for i in range(len(img_list)):
        img_list[i] = cv2.bitwise_and(img_list[i], img_list[i], mask=mask)
        base_cut = cv2.bitwise_and(base_cut, base_cut, mask=mask)
    ...
    while 1:
        ...
        if start and not finished:
            ...
            if shape == 0: # square
                ...
                mask = get_par_kernel(size)
                edited = np.zeros(base_img.shape, dtype=np.uint8)
                #cv2.imshow("mask", mask)
                for x in range(base_img.shape[0] // size):
                    print(x, "out of", base_img.shape[0] // size)
                    if finished:
                        break
                    for y in range(2 * base_img.shape[1] // size):
                        start = cv2.getTrackbarPos('Start', 'image')
                        if not start:
                            finished = 1
                        if finished:
